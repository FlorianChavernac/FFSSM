...
public class MoniteurTest {
    ...
    public void testEmployeurActuel() {
        assertEquals(p2.employeurActuel(), Optional.empty(), "Pas d'employeur");
        p2.nouvelleEmbauche(club, delivrance);
        assertEquals(p2.employeurActuel(), Optional.of(club), "l'employeur actuel n'est pas bon");
        lesEmbauches = new ArrayList<>();
        lesEmbauches.add(new Embauche(delivrance, p2, club));
        assertTrue(p2.emplois().equals(lesEmbauches));
